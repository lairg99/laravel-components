...
@php
    ...
    $colorStyle = match($color) {
        ...
        'tertiary' =>  'bg-none     hover:bg-gray-200 text-gray-400 hover:text-gray-500 focus:ring-gray-300 dark:none        dark:hover:bg-gray-900 dark:text-gray-400 dark:focus:ring-gray-600',
...
@if(! $can || auth()->user()->can(... $can))
    <button @if($route) onclick="window.location.href='{{ $route }}';" @endif type="{{ $type }}" {{ $attributes->merge(['class' => "{$baseStyle} {$sizeStyle} {$colorStyle}"]) }}>
